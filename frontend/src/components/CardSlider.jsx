                ...
                {type.map((item,index) => (
                    <div key={index} className="group">
                      ...
                      <div className={`cursor-pointer p-8 rounded-lg flex flex-col justify-center items-start gap-4 ${
                        currentSlide % type.length===index ? "bg-green text-white ":"bg-white text-green group-hover:bg-green group-hover:text-white"
                      }`}>

                        <p className={`font-bold text-lg ${
                        currentSlide % type.length===index ? "text-white ":" text-green group-hover:text-white"
                      }`}>
                      {item.title}
                      </p>
